...
const COLLECTORS = [
  ['Ramesh Kumar', 'collector@demo.com', 'hi', 'Connaught Place, New Delhi', 28.6139, 77.2090, '+91 98100 00001'],
  ['Sunita Devi', 'sunita@demo.com', 'hi', 'Okhla Industrial Area, South Delhi', 28.5355, 77.2426, '+91 98100 00002'],
  ['Imran Shaikh', 'imran@demo.com', 'mr', 'Wazirpur, North Delhi', 28.7041, 77.1668, '+91 98100 00003'],
  ['Lakshmi Bai', 'lakshmi@demo.com', 'hi', 'Sector 18, Noida', 28.5355, 77.3910, '+91 98100 00004'],
  ['Govind Meena', 'govind@demo.com', 'hi', 'Raj Nagar, Ghaziabad', 28.6692, 77.4538, '+91 98100 00005'],
  ['Prakash Jadhav', 'prakash@demo.com', 'mr', 'Cyber City, Gurugram', 28.4595, 77.0266, '+91 98100 00006'],
  ['Fatima Bano', 'fatima@demo.com', 'hi', 'Sector 15, Faridabad', 28.4089, 77.3178, '+91 98100 00007'],
  ['Deepak Yadav', 'deepak@demo.com', 'hi', 'Lajpat Nagar, South Delhi', 28.5672, 77.2100, '+91 98100 00008'],
  ['Sanjay More', 'sanjay@demo.com', 'mr', 'Dwarka, South West Delhi', 28.5965, 77.0498, '+91 98100 00009'],
  ['Kavita Sharma', 'kavita@demo.com', 'en', 'Karol Bagh, Central Delhi', 28.6512, 77.1911, '+91 98100 00010'],
]
...
function seed() {
  ...
  let uid = 2
  COLLECTORS.forEach(([name, email, lang, area, lat, lng, phone], i) => {
    db.users.push({ id: uid, email, phone, role: 'collector', name, language: lang })
    db.collectors.push({
      collector_id: i + 1, user_id: uid, display_name: name, language: lang,
      operating_location: area, latitude: lat, longitude: lng, phone,
  ...
  const complete = (lot, recycler, { stopAt = 'paid', anomalous = false, mode = 'cash' } = {}) => {
    ...
    // Every later stage implies the recycler accepted the lot — this mirrors
    // the backend, which only reveals the collector's phone from this point.
    lot.collector_contact = db.collectors.find((c) => c.collector_id === lot.collector_id)?.phone || null
  ...
  // A couple of live reverse-auction lots so the "Bidding Ground" grid has
  // something to demo immediately, not only after a collector starts a new
  // one from scratch.
  const auctionSeeds = [
    { collectorIdx: 4, category: cats[1], weight: 12.4, minutes: 18 },
    { collectorIdx: 8, category: cats[4], weight: 7.8, minutes: 42 },
  ]
  auctionSeeds.forEach(({ collectorIdx, category, weight, minutes }) => {
    const collector = db.collectors[collectorIdx % db.collectors.length]
    const lot = makeLot(collector, category, weight, 0)
    lot.auction_status = 'open'
    lot.auction_ends_at = new Date(t0 + minutes * 60000).toISOString()
    addEvent(lot.lot_id, 'AUCTION_STARTED',
      `Reverse auction open for ${minutes} min — recyclers bid, the highest bid at the deadline wins`,
      collector.display_name, t0)
    // Seed one opening bid from an eligible recycler so the highest-bid
    // readout isn't empty the very first time someone opens the dashboard.
    const bidder = pick(approved.filter((r) => r.accepted_materials.includes(category)))
    if (bidder) {
      const rate = Number(bidder.offered_rate[category] || currentRate[category] || 0)
      const amount = Math.round(rate * weight)
      db.offers.push({
        offer_id: db.offers.length + 1, lot_id: lot.lot_id, recycler_id: bidder.recycler_id,
        rate_per_kg: rate, amount, note: '', pickup_offered: bidder.pickup_available,
        status: 'PENDING', created_at: t0, updated_at: t0,
      })
      addEvent(lot.lot_id, 'OFFER_RECEIVED', `${bidder.name} offered ₹${rate}/kg (₹${amount})`, bidder.name, t0)
    }
  })
}
...
export function login(email, password) {
  ...
  db.session = {
    ...
    authorization_status: profile?.authorization_status ?? null,
  }
...
export function registerCollector({ name, email, language, operating_location, latitude, longitude }) {
  ...
  const collector = {
    ...
    phone: '', authorization_status: 'pending',
    created_at: now(),
  }
...
/* ------------------------------------------------------- collector OTP */
// No SMS gateway in the demo — the code is generated here and handed straight
// back to the caller (same shape as the real backend's /otp/request).
const otpStore = {}
const DEMO_FALLBACK_OTP = '123456'
...
export function requestOtp(phone) {
  ...
  const code = String(Math.floor(100000 + rnd() * 900000))
  otpStore[p] = { code, expires: Date.now() + OTP_TTL_MS }
  return {
    phone: p, demo_otp: code, expires_in: OTP_TTL_MS / 1000,
    note: `Demo mode — no SMS is actually sent. Use the code above (or ${DEMO_FALLBACK_OTP}) to continue.`,
  }
}

export function verifyOtp(phone, otpCode, extra = {}) {
  seed()
  const p = normalizePhone(phone)
  const entry = otpStore[p]
  const ok = otpCode === DEMO_FALLBACK_OTP || (entry && entry.code === otpCode && entry.expires > Date.now())
  if (!ok) {
    const err = new Error('Incorrect or expired OTP')
    err.status = 400
    throw err
  }
  let user = db.users.find((u) => u.phone === p && u.role === 'collector')
  if (!user) {
    const id = Math.max(...db.users.map((u) => u.id)) + 1
    const name = (extra.name && extra.name.trim()) || `Collector ${p.slice(-4)}`
    user = { id, email: `phone-${p.replace('+', '')}@collector.local`, phone: p, role: 'collector', name, language: 'hi' }
    db.users.push(user)
    db.collectors.push({
      collector_id: db.collectors.length + 1, user_id: id, display_name: name, language: 'hi',
      operating_location: extra.operating_location || 'Pune',
      latitude: extra.latitude || 18.5204, longitude: extra.longitude || 73.8567,
      phone: p, authorization_status: 'pending', created_at: now(),
    })
  } else if (extra.name && extra.name.trim() && extra.name.trim() !== user.name) {
    user.name = extra.name.trim()
    const col = db.collectors.find((c) => c.user_id === user.id)
    if (col) col.display_name = user.name
  }
...
export function createLot(payload) {
  ...
  const lot = {
    ...
    recycler_id: null, recycler_name: null, match_score: 0, status: 'PRICE_ESTIMATED',
    collector_contact: null,
    // Reverse-auction bidding (mirrors the backend Lot model): 'none' unless
    // the collector opted into a timed auction below.
    auction_status: 'none', auction_ends_at: null,
    created_at: created,
  }
  if (payload.auction_minutes) {
    lot.auction_status = 'open'
    // Stored as an ISO string (not a raw epoch number) because the UI's
    // AuctionCountdown/PriorityCountdown parse this the same way it parses
    // the real backend's serialized datetimes.
    lot.auction_ends_at = new Date(created + payload.auction_minutes * 60000).toISOString()
  }
  db.lots.push(lot)
  ...
  if (payload.auction_minutes) {
    addEvent(lot.lot_id, 'AUCTION_STARTED',
      `Reverse auction open for ${payload.auction_minutes} min — ` +
      'recyclers bid, the highest bid at the deadline wins', collector.display_name)
  }
  return lot
}
...
export function listLots() {
  ...
  if (role === 'collector') {
    ...
    rows = db.lots.filter((l) => l.collector_id === c.collector_id)
  } else if (role === 'recycler') {
    const r = currentRecycler()
    rows = db.lots.filter((l) => l.recycler_id === r.recycler_id)
  } else {
    rows = [...db.lots]
  }
  rows.forEach(resolveIfExpired)
...
export function decideOnLot(lotId, decision) {
  ...
  if (decision === 'accept') {
    ...
    // Once the recycler accepts, hand over the collector's phone number so
    // the two sides can coordinate the pickup.
    const collector = db.collectors.find((c) => c.collector_id === lot.collector_id)
    lot.collector_contact = collector?.phone || null
    if (txnIndex > -1) {
...
export function adminStats() {
  ...
  return {
    ...
    authorized_collectors: db.collectors.filter((c) => c.authorization_status === 'approved').length,
    pending_collectors: db.collectors.filter((c) => c.authorization_status === 'pending').length,
...
export function adminCollectors() { seed(); return db.collectors }

export function verifyCollector(collectorId, decision) {
  const c = db.collectors.find((x) => x.collector_id === collectorId)
  c.authorization_status = decision
  return c
}
...
/** Highest live (PENDING) bid on a lot, optionally excluding one recycler's
 * own bid — mirrors `services/auction.py::highest_bid` on the backend. */
export function highestBid(lotId, excludeRecyclerId = null) {
  const rows = db.offers.filter(
    (o) => o.lot_id === lotId && o.status === 'PENDING' &&
      (excludeRecyclerId == null || o.recycler_id !== excludeRecyclerId)
  )
  if (!rows.length) return null
  return rows.reduce((a, b) => (b.amount > a.amount ? b : a))
}

/** Assign `offer`'s recycler to `lot` and open a Transaction. Shared by the
 * collector's manual "accept" and by automatic auction resolution — mirrors
 * `services/auction.py::finalize_offer`. */
function finalizeOffer(lot, offer, actor) {
  ...
  db.offers.filter((o) => o.lot_id === lot.lot_id && o.offer_id !== offer.offer_id && o.status === 'PENDING')
    .forEach((o) => { o.status = 'DECLINED'; o.updated_at = now() })
  const txn = {
    transaction_id: db.transactions.length + 1, lot_id: lot.lot_id, collector_id: lot.collector_id,
  ...
  db.transactions.push(txn)
  addEvent(lot.lot_id, 'OFFER_ACCEPTED', `${rec.name} at ₹${offer.rate_per_kg}/kg (₹${offer.amount})`, actor)
  addEvent(lot.lot_id, 'RECYCLER_MATCHED', `${rec.name} accepted`, actor)
  addEvent(lot.lot_id, 'HANDOVER_PENDING', 'Waiting for recycler to scan the lot QR')
  return txn
}

/** If `lot`'s auction window has passed and it hasn't been resolved yet,
 * auto-accept the highest pending bid (or close with no winner). There is
 * no background timer in the demo engine either, so — exactly like the
 * backend — this is called lazily from every read/write path that touches
 * a lot. Returns true if the lot's state changed. */
export function resolveIfExpired(lot) {
  if (lot.auction_status !== 'open' || !lot.auction_ends_at || lot.recycler_id) return false
  if (Date.now() < new Date(lot.auction_ends_at).getTime()) return false
  const top = highestBid(lot.lot_id)
  lot.auction_status = 'closed'
  if (top) {
    finalizeOffer(lot, top, 'Auction')
    addEvent(lot.lot_id, 'AUCTION_ENDED',
      `Auction closed — winning bid ₹${top.amount} (₹${top.rate_per_kg}/kg)`, 'system')
  } else {
    addEvent(lot.lot_id, 'AUCTION_ENDED', 'Auction closed with no bids', 'system')
  }
  return true
}
...
export function openLots() {
  ...
  return db.lots
    ...
    // An expired auction may just have been won by someone else — skip it
    // rather than showing a lot that is no longer actually open.
    .filter((l) => !resolveIfExpired(l))
    ...
    .map((l) => {
      ...
      const top = l.auction_status === 'open' ? highestBid(l.lot_id) : null
      return {
        ...
        highest_bid: top ? top.amount : null,
        highest_bid_rate: top ? top.rate_per_kg : null,
      }
    })
...
export function makeOffer(lotId, { rate_per_kg, note = '', pickup_offered = false }) {
  ...
  resolveIfExpired(lot)
  if (!OPEN_STATUSES.includes(lot.status) || lot.recycler_id) {
    const e = new Error('This lot is no longer open for offers'); e.status = 400; throw e
  }
  const rec = currentRecycler()
  const amount = Math.round(rate_per_kg * lot.weight)
  if (lot.auction_status === 'open') {
    const top = highestBid(lotId, rec.recycler_id)
    if (top && amount <= top.amount) {
      const e = new Error(`Bid must beat the current highest bid of ₹${top.amount}`)
      e.status = 400
      throw e
    }
  }
...
export function acceptOffer(offerId) {
  ...
  if (offer.status !== 'PENDING') {
    const e = new Error(`Offer is already ${offer.status}`); e.status = 400; throw e
  }
  if (!OPEN_STATUSES.includes(lot.status) || lot.recycler_id) {
    const e = new Error('This lot already has a recycler'); e.status = 400; throw e
  }
  const collector = db.collectors.find((c) => c.collector_id === lot.collector_id)
  ...
  if (lot.auction_status === 'open') lot.auction_status = 'closed'
  const txn = finalizeOffer(lot, offer, collector?.display_name || lot.collector_name)
  return { ...lot, qr_payload: `/verify/${lot.lot_id}`, transaction_id: txn.transaction_id,
