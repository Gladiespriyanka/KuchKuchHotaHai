...
/**
 * Recycler profile screen. Allows editing name, phone number, and language.
 * Updates are sent via PATCH /api/auth/me and reflected across the app.
 */
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="plate-lg p-4">
        <label className="eyebrow" htmlFor="rname">{t('yourName')}</label>
        <input
          id="rname" className="field mt-1" value={name}
          onChange={(e) => setName(e.target.value)} maxLength={120}
        />

        <label className="eyebrow mt-4" htmlFor="rphone">
          {t('phoneNumber')}
        </label>
        <input
          id="rphone" className="field mt-1" value={phone}
      ...
      <div className="plate p-3 text-sm">
        <dl className="space-y-1">
          ...
          <Row label="Recycler ID" value={user?.profile_id} />
        </dl>
      </div>
