...
export function Verification() {
  const { t } = useI18n()
  const [tab, setTab] = useState('recyclers')
  const [rows, setRows] = useState(null)
  const [colRows, setColRows] = useState(null)
  const [busy, setBusy] = useState(null)
  const [error, setError] = useState('')

  const load = () => admin.recyclers().then(setRows).catch((e) => { setError(e.message); setRows([]) })
  const loadCollectors = () =>
    admin.collectors().then(setColRows).catch((e) => { setError(e.message); setColRows([]) })
  ...
  async function decideCollector(id, decision) {
    setBusy(`c${id}`)
    try {
      await admin.verifyCollector(id, decision)
  ...
  if (!rows || !colRows) return <Loading />
  const pending = rows.filter((r) => r.authorization_status === 'pending')
  const pendingCollectors = colRows.filter((c) => c.authorization_status === 'pending')
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate2">
          {tab === 'recyclers' ? t('verificationIntro') : t('collectorApprovalIntro')}
        </p>
      </div>
      ...
      <div className="flex gap-2">
        <button
          type="button"
          className={`btn py-2 text-sm ${tab === 'recyclers' ? 'bg-board text-white' : 'bg-white'}`}
          onClick={() => setTab('recyclers')}
        >
          {t('recyclersTab')} {pending.length > 0 && `(${pending.length})`}
        </button>
        <button
          type="button"
          className={`btn py-2 text-sm ${tab === 'collectors' ? 'bg-board text-white' : 'bg-white'}`}
          onClick={() => setTab('collectors')}
        >
          {t('collectorsTab')} {pendingCollectors.length > 0 && `(${pendingCollectors.length})`}
        </button>
      </div>
      ...
      {tab === 'recyclers' ? (
        ...
        <>
          {pendingCollectors.length > 0 && (
            <section>
              <h2 className="eyebrow mb-2">{t('pendingLabel')} ({pendingCollectors.length})</h2>
              <div className="grid gap-3 md:grid-cols-2">
                {pendingCollectors.map((c) => (
                  <div key={c.collector_id} className="plate-lg p-4">
                    <div className="font-display text-xl">{c.display_name}</div>
                    <div className="text-sm text-slate2">{c.operating_location}</div>
                    <dl className="mt-2 space-y-1 text-sm">
                      <Line label={t('phoneLabel')} value={c.phone || '—'} />
                      <Line label={t('areaLabel')} value={c.operating_location} />
                    </dl>
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <button className="btn-primary py-2" disabled={busy === `c${c.collector_id}`}
                              onClick={() => decideCollector(c.collector_id, 'approved')}>
                        <Check size={16} /> {t('approve')}
                      </button>
                      <button className="btn-ghost py-2" disabled={busy === `c${c.collector_id}`}
                              onClick={() => decideCollector(c.collector_id, 'rejected')}>
                        <X size={16} /> {t('reject')}
                      </button>
                    </div>
                  </div>
          ...
          <section>
            <h2 className="eyebrow mb-2">{t('allCollectors')}</h2>
            <Table
              head={[t('nameLabel'), t('phoneLabel'), t('areaLabel'), t('statusLabel'), '']}
              rows={colRows.map((c) => [
                c.display_name,
                <span className="num">{c.phone || '—'}</span>,
                c.operating_location,
                <span className={`chip ${c.authorization_status === 'approved' ? 'bg-board text-white'
                  : c.authorization_status === 'pending' ? 'bg-brass' : 'bg-copper text-white'}`}>
                  {c.authorization_status}
                </span>,
                c.authorization_status === 'approved' ? (
                  <button className="btn-ghost px-2 py-1 text-xs" onClick={() => decideCollector(c.collector_id, 'rejected')}>
                    {t('reject')}
                  </button>
                ) : (
                  <button className="btn-ghost px-2 py-1 text-xs" onClick={() => decideCollector(c.collector_id, 'approved')}>
                    {t('approve')}
                  </button>
                ),
              ])}
