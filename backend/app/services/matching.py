...
def resolve_lot_city(lot) -> str:
    """Best-effort city for a lot that has no `city` column value set."""
    city = (getattr(lot, "city", "") or "").strip()
    if not city and getattr(lot, "location", ""):
        city = lot.location.split(",")[-1].strip()
    return city


def in_service_area(lot, r: Recycler, lot_city: str | None = None) -> tuple[bool, float | None]:
    """Whether recycler `r` is within reach of `lot`.

    Falls back to a same-city check when the lot has no GPS fix yet (created
    offline, or before the browser granted location) instead of measuring a
    haversine distance from (0, 0) — a point in the Gulf of Guinea that is
    "far" from literally every recycler on Earth. Without this fallback, a
    lot created with no coordinates would silently vanish from every
    recycler's Bidding Ground, including ones in the exact same city.

    Returns (in_range, distance_km). distance_km is None when the check was
    city-based rather than GPS-based, since no real distance was computed.
    """
    ...
    if has_coords:
        ...
        return distance <= max(r.service_area_km, 5) * 1.5, distance
    city = lot_city if lot_city is not None else resolve_lot_city(lot)
    if city and (r.city or "").strip() and r.city.strip() != city:
        return False, None
    return True, None


def auction_in_service_area(lot, r: Recycler) -> tuple[bool, float | None]:
    """Use the declared lot city as the auction location when available.

    Auction lots can be created with a stale device GPS fix while the
    collector's selected area is still correct. A live auction must not
    disappear from that city's bidding ground because those two signals
    disagree; normal GPS filtering remains the fallback for city-less lots.
    """
    city = resolve_lot_city(lot)
    recycler_city = (r.city or "").strip()
    if city and recycler_city and city.casefold() == recycler_city.casefold():
        return True, None
    return in_service_area(lot, r)
...
def match_recyclers(db: Session, lot, limit: int = 6) -> list[dict]:
    ...
    for r in candidates:
        ...
        if rate <= 0:
            continue
        in_range, distance = in_service_area(lot, r, lot_city)
        if not in_range:
            continue
