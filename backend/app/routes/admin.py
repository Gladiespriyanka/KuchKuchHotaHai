...
from ..services.common import collector_dict, lot_dict, recycler_dict
...
def stats(user: User = Depends(require_role("admin")), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "authorized_collectors": db.query(Collector).filter(
            Collector.authorization_status == "approved"
        ).count(),
        "pending_collectors": db.query(Collector).filter(
            Collector.authorization_status == "pending"
        ).count(),
...
def all_collectors(status_filter: str | None = None,
                    user: User = Depends(require_role("admin")),
                    db: Session = Depends(get_db)):
    q = db.query(Collector)
    if status_filter:
        q = q.filter(Collector.authorization_status == status_filter)
    return [collector_dict(c) for c in q.order_by(Collector.collector_id).all()]


@router.post("/collectors/{collector_id}/verify")
def verify_collector(collector_id: int, decision: str,
                      user: User = Depends(require_role("admin")),
                      db: Session = Depends(get_db)):
    if decision not in ("approved", "rejected"):
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "decision must be approved or rejected")
    col = db.get(Collector, collector_id)
    if not col:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Collector not found")
    col.authorization_status = decision
    db.commit()
    return collector_dict(col)
