...
def run(full_history: bool = False, limit_lots: int | None = None) -> None:
    ...
    for i, row in enumerate(collectors):
        ...
        # collectors.csv carries no phone column — synthesise a stable demo
        # number from the collector's sequence so phone-login is usable.
        phone = f"+91 9{80000000 + i:08d}"
        user = User(
            email=email, password_hash=hash_password(DEFAULT_PASSWORD),
            role="collector", name=row["display_name"],
            language=row.get("preferred_language", "hi"), phone=phone,
        )
        db.add(user)
        db.flush()
        ...
        collector = Collector(
            ...
            latitude=lat, longitude=lng, phone=phone, authorization_status="approved",
    ...
    # ----------------------------------------------------------- recyclers
    # Every recycler gets a login, exactly like every collector does above.
    # Earlier this only created a User for the one recycler named in
    # demo_scenario.json — every other facility had user_id=None with no way
    # to sign in at all. That made it impossible to demo competitive bidding
    # (the "Bidding Ground") with more than one recycler, and any login
    # other than recycler@demo.com looked like a bug ("wrong password")
    # when it was actually just a missing account.
    recyclers = read("recyclers.csv")
    rec_by_ext: dict[str, Recycler] = {}
    for row in recyclers:
        ext = row["recycler_id"]
        is_demo = ext == demo_recycler_id
        email = "recycler@demo.com" if is_demo else f"{ext.lower()}@demo.com"
        u = User(email=email, password_hash=hash_password(DEFAULT_PASSWORD),
                 role="recycler", name=row["name"], language="mr" if is_demo else "en")
        db.add(u)
        db.flush()
        user_id = u.id
    ...
    print(f"\nEvery collector and every recycler also has its own login: "
          f"<id>@demo.com (e.g. {recyclers[0]['recycler_id'].lower()}@demo.com), password123.")
    print("  Use several different recycler logins at once to see the Bidding "
          "Ground with real competing bids.")
