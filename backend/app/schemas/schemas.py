...
class OtpRequestIn(BaseModel):
    """Step 1 of collector phone login: ask for a code."""
...
class RegisterIn(BaseModel):
    ...
    # Collectors register through phone + OTP now (see OtpVerifyIn); this
    # email/password path stays for recyclers.
    role: Literal["recycler"] = "recycler"
...
class PickupScheduleIn(BaseModel):
    ...
    radius_km: float = Field(default=6.0, gt=0, le=50)


class BookSlotIn(BaseModel):
    lot_id: str
    occurrence_date: str  # "YYYY-MM-DD" — must be a future date matching the schedule's weekday
